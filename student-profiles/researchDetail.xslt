      ...
      <xsl:choose>
        ...
        <xsl:when test="count($research) &gt;0">
          ...
          <MainCol>
            ...
            <ul>
              <xsl:for-each select="$research/INTELLCONT_AUTH">
                <xsl:choose>
                  <xsl:when test="FACULTY_NAME != '' and fn:isCurrentFaculty(key('studentList', FACULTY_NAME))">
                    <li>
                      <a>
                        <xsl:attribute name="href">
                          <xsl:value-of select="fn:getStudentProfileLink(key('studentList', FACULTY_NAME))"/>
                        </xsl:attribute>
                        <span xml:space="preserve">
                          <xsl:value-of select="fn:getPreferredFirstName(key('studentList', FACULTY_NAME))"/>
                          <xsl:text/>
                          <xsl:value-of select="key('studentList', FACULTY_NAME)/PCI/LNAME"/>
                        </span>
                      </a>
                    </li>
                  </xsl:when>
          ...
          <RightCol>
            ...
            <xsl:for-each select="$research/INTELLCONT_AUTH">
              <xsl:if test="FACULTY_NAME != '' and fn:isCurrentStudent(key('studentList', FACULTY_NAME))">
                <div class="imageContainer">
                  <div class="image">
                    <a href="{fn:getStudentProfileLink(key('studentList', FACULTY_NAME))}">
                      <img border="0" onerror="this.style.display='none'" src="/images/people/{key('studentList', current()/FACULTY_NAME)/@username}.jpg"/>
