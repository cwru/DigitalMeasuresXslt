    ...
    <Data>
    	<MastHead>
            <div class="row masthead-profile-name">
                <div class="col-xs-12">
                    <h1><xsl:value-of select="fn:getPreferredFirstName(.)" />&#160;<xsl:value-of select="PCI/LNAME" /></h1>
                </div>
            </div>
            <div class="row masthead-profile-details">
                <div class="col-sm-2">
                    <!-- Photo -->	
                    <img src="/images/people/{@username}.jpg" class="img-responsive" style="border:0;" onerror="this.style.visibility='hidden';">
                ...
                <xsl:copy-of select="fn:getDegreesText(.)" />
				...
				<xsl:for-each select="PASTHIST[
                	...
                	<br /><xsl:value-of select="TITLE" />, <xsl:value-of select="ORG" />
                </xsl:for-each>
                    ...
                    <xsl:for-each select="ADMIN/ADMIN_DEP[
                        ...
                        <xsl:value-of select="../RANK" /> <xsl:if test="../RANK != 'Professor for the Practice of'">,</xsl:if>&#160;<xsl:value-of select="DEP" /><br />
                    </xsl:for-each>
                    ...
                    <xsl:if test="PCI/EMAIL != ''">
                        <a><xsl:attribute name="href">mailto:<xsl:value-of select="PCI/EMAIL" /></xsl:attribute><xsl:value-of select="PCI/EMAIL" /></a><br />
                    </xsl:if>
        ...
        <MainCol>
            ...
            <xsl:copy-of select="fn:lineBreaksToParagraphs(WEB_PROFILE/BIO_PROFILE)" />
            </xsl:if>    
        
            <!-- Website -->
            <xsl:if test="PCI/WEBSITE != ''">
                <a href="http://{PCI/WEBSITE}" target="_blank">Personal Website</a><br />
            </xsl:if>
            <!-- CV -->
            ...
            <xsl:if test="EDUCATION">
                ...
                <xsl:for-each select="EDUCATION">
					...
					<xsl:if test="YR_COMP &lt; format-date(current-date(),'[Y0001]')">
                    	<span><xsl:value-of select="DEG" />, <xsl:value-of select="SCHOOL" />, <xsl:value-of select="YR_COMP" /></span><br />
					</xsl:if>
                </xsl:for-each>
                </p>
            </xsl:if>
                ...
                <xsl:for-each select="INTELLCONT[PROFILE = 'Yes' and (STATUS='Published' or STATUS='Accepted' or STATUS='Working Paper Completed')]">
                    ...
                    <li style="word-wrap: break-word; margin-bottom: 15px;">
                        ...
                        <xsl:if test="ABSTRACT != ''">
                            <a href="/students/research/{./@id}"><em xml:space="preserve"> <xsl:value-of select="TITLE" /></em></a> 
                        </xsl:if>
                        <xsl:if test="ABSTRACT = ''">
